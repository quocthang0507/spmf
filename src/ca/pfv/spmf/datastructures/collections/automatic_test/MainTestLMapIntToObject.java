...
/**
 * A class for testing the LMapIntToObject class.
 *
 * @author Philippe Fournier-Viger, 2023
 */
public class MainTestLMapIntToObject {
    ...
    private static void runExperiment(int bucketCount) {
        MapIntToObject<Integer> hashMap = new LMapIntToObject<Integer>(bucketCount);
        ...
        CheckResults.checkResult(hashMap.containsKey(5));
        CheckResults.checkResult(hashMap.containsKey(6));
        CheckResults.checkResult(hashMap.get(1) == null);
        CheckResults.checkResult(hashMap.get(2) == 200);
        CheckResults.checkResult(hashMap.get(3) == 300);
        CheckResults.checkResult(hashMap.get(4) == 400);
        CheckResults.checkResult(hashMap.get(5) == 500);
        ...
        CheckResults.checkResult(hashMap.containsKey(6));
        CheckResults.checkResult(hashMap.get(1) == null);
        CheckResults.checkResult(hashMap.get(2) == null);
        CheckResults.checkResult(hashMap.get(3) == 300);
        CheckResults.checkResult(hashMap.get(4) == 400);
        CheckResults.checkResult(hashMap.get(5) == 500);
        CheckResults.checkResult(hashMap.get(6) == 600);
        ...
        int count = 0;
        while (iter.hasNext()) {
            MapEntryIntToObject node = iter.next();
        ...
        //================ CODE TO VERIFY THAT THE ITERATOR IS WORKING PROPERLY IF WE REMOVE TWO VALUES
        System.out.println("ITERATING OVER THE KEY,VALUES");
        EntryIterator iter2 = hashMap.iterator();
        Set<MapEntryIntToObject> hashSetResults2 = new HashSet<>();
        int count2 = 0;
        while (iter2.hasNext()) {
            MapEntryIntToObject node = iter2.next();
            count2++;
        ...
        int count3 = 0;
        while (iter3.hasNext()) {
            MapEntryIntToObject node3 = iter3.next();
        ...
        CheckResults.checkResult(x.getKey() == 9 && (Integer) x.getValue() == 90);
        ...
        CheckResults.checkResult(x6.getKey() == 9 && (Integer) x6.getValue() == 90);
