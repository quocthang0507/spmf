...
public class AlgoECHUM {
     ...
     */
    public Itemsets runAlgorithm(int minUtil, double minCor, String inputPath, String outputPath, boolean activateTransactionMerging, int maximumTransactionCount, boolean activateSubtreeUtilityPruning) throws IOException {
...
//		this.minUtil = minUtil*dataset.getTotalUtility();
...
//		System.out.println("MINUTIL IS "+this.minUtil);
        ...
        //System.out.println(Arrays.toString(itemsToKeep.toArray()));
...
//		System.out.println("DAtaset before removing Unpromising Items :");
//		System.out.println(dataset.toString());
...
//   		System.out.println("Dataset after Transaction Merging is : ");
//  		System.out.println(dataset.toString());
            ...
            for (int p = 0; p < dataset.getTransactions().size(); p++) {
                // if the transaction length is 0, increase the number of empty transactions
                Transaction transaction2 = dataset.getTransactions().get(p);
                if (transaction2.items.length == 0) {
            ...
            //System.out.println(emptyTransactionCount);
        }
...
//    	//======
        ...
        if (activateSubtreeUtilityPruning) {
            ...
            backtrackingECHUM(dataset.getTransactions(), itemsToKeep, itemsToExplore, 0);
        } else {
            // We call the recursive algorithm with the database and secondary items
            backtrackingECHUM(dataset.getTransactions(), itemsToKeep, itemsToKeep, 0);
        }
    ...
    private void backtrackingECHUM(List<Transaction> transactionsOfP,
...
//    	System.out.println("Transactions of P");
//    	System.out.println(transactionsOfP);
        // ========  for each frequent item  e  =============
        String l = "";
        for (int q = 0; q < prefixLength; q++) {
            l = l + Integer.toString(temp[q]);
        }
//        System.out.println("L is "+l);
//        for(Transaction transaction : transactionsOfP) {
// 
//        	for(int k=0;k<transaction.items.length;k++)
//        	{
//        		System.out.print(transaction.items[k]+" ");
//        	}
//        	System.out.println();
//        }
        ...
        for (Transaction transaction : transactionsOfP) {
            if (l != "") {
                for (int k = 0; k < transaction.items.length; k++) {
                    ...
                    String str = Integer.toString(newNamesToOldNames[transaction.items[k]]);
                    boolean flag = true;
                    for (int j = 0; j < prefixLength; j++) {
                        if (temp[j] == newNamesToOldNames[transaction.items[k]]) {
                            flag = false;
                            break;
                        }
                    }
                    if (flag == false) continue;
                    if (mp.containsKey(l + str)) {
                        int h = mp.get(l + str);
                        mp.put(l + str, h + transaction.mergecount);
                    } else {
                        mp.put(l + str, transaction.mergecount);
                    }
                }
            }
        }
                ...
                if (positionE > -1) {
                    ...
                    if (transaction.getLastPosition() == positionE) {
                        ...
                        if (activateTransactionMerging && MAXIMUM_SIZE_MERGING >= (transaction.items.length - positionE)) {
                            ...
                            Transaction projectedTransaction = new Transaction(transaction, positionE, transaction.mergecount);
                            ...
                            if (previousTransaction == null) {
                                ...
                                if (consecutiveMergeCount == 0) {
                                    ...
                                    int temp_merge_count = previousTransaction.mergecount;
                                    ...
                                    previousTransaction.mergecount = projectedTransaction.mergecount + temp_merge_count;
                                    ...
                                    previousTransaction.mergecount += projectedTransaction.mergecount;
                                }
...
//	    	   if(mp.containsKey(l))
//	    	   {
//	    		   if(mp.containsKey(l+e))
//	    		   {
//	    			   Integer zi = mp.get(l+e);
//	    			   Integer ti = mp.get(l);
//		    		   int x=ti.intValue();
//		    		   z=x;
//		    		   int mini = Math.min(con+zi,x);
//		    		   mp.put(l+temp[prefixLength],mini);
//		    		   System.out.println("String is "+l+" "+temp[prefixLength]+" CON ISabc " + con+" "+zi + " and x is " + z);
...
//	    		   int x=ti.intValue();
//	    		   z=x;
//	    		   int mini = Math.min(con,x);
//	    		   mp.put(l+temp[prefixLength],mini);
//	    		   System.out.println("String is "+l+temp[prefixLength]+" CON IS " + con + " and x is " + z);
//	    	       System.out.println("MAP IS :" +mp);
//	    		   }
//	    	   }
//	    	   
//	       }
            ...
            for (int p = 0; p <= prefixLength; p++) {
                String we = Integer.toString(temp[p]);
                fe = fe + we;
                double val = mp.get(we);
                KulcPe += (1 / val);
            }
            double val = mp.get(fe);
            KulcPe = KulcPe * val;
            double leng = prefixLength + 1;
            KulcPe = KulcPe / (leng);
            // if the utility of PU{e} is enough to be a high utility itemset
            if (utilityPe >= minUtil && KulcPe >= minCor) {
                // output PU{e}
                output(prefixLength, utilityPe, KulcPe);
            }
            if (KulcPe < minCor) {
                continue;
            }
            ...
            if (activateSubtreeUtilityPruning) {
                ...
                backtrackingECHUM(transactionsPe, newItemsToKeep, newItemsToExplore, prefixLength + 1);
            } else {
                // if sub-tree utility pruning is deactivated, we consider secondary items also
                // as primary items
                backtrackingECHUM(transactionsPe, newItemsToKeep, newItemsToKeep, prefixLength + 1);
            }
        }
...
//    	System.out.println("Transacion Head: "+ j);
//    	
//    	System.out.println(itemsToKeep);
//    	
...
//    	{
//			System.out.println("Transacion is " + transaction.toString());
//    	}

//    	System.out.println("Head of Utility Bin Arrays Before");
...
//		System.out.println("Head of Utility Bin Arrays");
//		System.out.println(Arrays.toString(utilityBinArraySU));
//		System.out.println(Arrays.toString(utilityBinArrayLU));
     ...
     */
    private void output(int tempPosition, int utility, double KulcPe) throws IOException {
        patternCount++;
