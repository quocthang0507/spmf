...
/* This file is copyright (c) 2008-2013 Azadeh Soltani, Philippe Fournier-Viger, Srinivas Paturu
 *
 ...
 * This implementation is enhanced by SRINIVAS PATURU for performance enhancement.
 *
 * @author Azadeh Soltani, Philippe Fournier-Viger, Srinivas Paturu
 * @see Itemset
 */
public class AlgoMSAprioriSrinivas {
     ...
     * @param hashUpto the parameter for hashing itemsets up to a given size in the first pass of database scan
     * @throws IOException exception if error while writing the output file
     */
    public void runAlgorithm(String input, String output,
                             double beta, double LS, int hashUpto) throws IOException, Exception {
        ...
        // map to store the bitset hash code and support of item sets
        @SuppressWarnings("unchecked")
        Map<BitSet, Integer>[] mapHashCounts
                = (HashMap<BitSet, Integer>[]) new HashMap[hashUpto + 1];
        for (int i = 2; i < mapHashCounts.length; i++)
            mapHashCounts[i] = new HashMap<BitSet, Integer>();
        ...
        while (((line = reader.readLine()) != null)) {
            ...
            // now compute the hash code of C2, C3 ... Ck itemsets here
            // and store the supports in the hash map
        ...
        if (maxItemsetSize > 1) {
            ...
            do {
                ...
                if (k == 2) {
                    ...
                    level = generateCandidate2(F, mapHashCounts[k], maxItemID);
                    mapHashCounts[k] = null;
                } else if (k <= hashUpto && k != 2) {
                    level = generateCandidateSizeK(level, transactionCount,
                            mapHashCounts[k], maxItemID);
                    mapHashCounts[k] = null;
                } else {
     ...
     * @param mapCkCount a map indicating the support of each item set (key: item set hash code, value: support)
     * @return the set of frequent item sets of size 2
     */
    private List<Itemset> generateCandidate2(List<Integer> frequent1,
                                             Map<BitSet, Integer> mapCkCount, int maxItemId)
            throws IOException {
        ...
        for (int i = 0; i < frequent1.size(); i++) {
            ...
            for (int j = i + 1; j < frequent1.size(); j++) {
                ...
                // get the support of C2 from the has table
                ...
                if (!mapCkCount.containsKey(ckhash))
                    continue;
                else
                    support = mapCkCount.get(ckhash);

                if (support == 0 || support < MIS[item1])
                    continue;
                else {
    ...
    /**
     * Compute the hash code of itemsets of size up to a given size in the given
     * transaction and preserve the hash code and support in the hash map
     *
     * @param transaction
     * @param hashUptoSize
     * @param mapHashCounts
     * @throws HashCodeOverflowException
     */
    private void computeHashedSupport(Integer[] transaction, int hashUptoSize,
                                      Map<BitSet, Integer>[] mapHashCounts, int maxItemId)
            throws Exception {
        // compute and hash supports for item set sizes up to k
        for (int k = 2; k < hashUptoSize + 1 && k < transaction.length + 1; k++) {
            Integer[][] CkItemsets
                    = new CombinationGenerator().generateCombinations(transaction, k);
            for (int i = 0; i < CkItemsets.length; i++) {
                Integer[] CkItemset = CkItemsets[i];

                // compute the hash code of this item set using Bit set
                BitSet ckhash = new BitSet(maxItemId);
                for (int j = 0; j < CkItemset.length; j++)
                    ckhash.set(CkItemset[j]);

                // increase the support of C2, C3 ... Ck
    ...
    /**
     * this function would generate frequent item sets up to a given size using the
     * hash table that has the preserved supports from the initial scan of the
     * transaction database.
     *
     * @param levelK_1   item sets of size k-1
     * @param n          transaction count for calculating the support
     * @param mapCkCount hash table that has the supports preserved
     * @return the frequent item sets of size k
     * @throws IOException
     * @throws Exception
     */
    protected List<Itemset> generateCandidateSizeK(List<Itemset> levelK_1,
                                                   int n, Map<BitSet, Integer> mapCkCount, int maxItemId)
            throws IOException, Exception {
        List<Itemset> candidates = new ArrayList<Itemset>();

        // Search all item set pairs in levelK_1 satisfying join property
        loop1:
        for (int i = 0; i < levelK_1.size(); i++) {
            int[] itemset1 = levelK_1.get(i).getItems();
            loop2:
            for (int j = i + 1; j < levelK_1.size(); j++) {
                ...
                // join property candidate is found
                int[] newItemset = new int[itemset1.length + 1];
                System.arraycopy(itemset1, 0, newItemset, 0, itemset1.length);
                newItemset[itemset1.length] = itemset2[itemset2.length - 1];

                // Check for F2, F3, F4...Fk constraint's
                // compute the hash code of this item set using Bit set
                BitSet ckhash = new BitSet(maxItemId);
                for (int k = 0; k < newItemset.length; k++)
                    ckhash.set(newItemset[k]);
                ...
                if (support == 0 || support < MIS[newItemset[0]])
                    continue;
