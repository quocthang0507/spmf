...
/**
 * This is an implementation of the "PHMN" and PHMN+ algorithms for Periodic High-Utility Itemsets Mining with negative utility
 * published in the EAAI journal
 *
 * @author Lai et al., modified from the code of PHM from SPMF.
 * @see UtilityListPHMN
 * @see ElementPHMN
 */
public class AlgoPHMN {
    ...
    /****** if true, PHMN+ is run instead of PHMN****/
    boolean improve;
    /**** minutil threshold ****/
    int minUtil;
    /**
     ...
     */
    public void runAlgorithm(boolean improve, String input, String output, int minUtility, int minPeriodicity, int maxPeriodicity, int minAveragePeriodicity, int maxAveragePeriodicity) throws IOException {
        // reset maximum
...
//						+ " avg periodicity: " + itemInfo.averagePeriodicity
        ...
        List<UtilityListPHMN> listOfUtilityLists = new ArrayList<UtilityListPHMN>();
        // CREATE A MAP TO STORE THE UTILITY LIST FOR EACH ITEM.
        // Key : item    Value :  utility list associated to that item
        Map<Integer, UtilityListPHMN> mapItemToUtilityList = new HashMap<Integer, UtilityListPHMN>();
        ...
        for (Integer item : mapItemToTWU.keySet()) {
            ...
            if (itemInfo.support >= supportPruningThreshold &&
                ...
                UtilityListPHMN uList = new UtilityListPHMN(item);
        ...
        Collections.sort(listOfUtilityLists, new Comparator<UtilityListPHMN>() {
            public int compare(UtilityListPHMN o1, UtilityListPHMN o2) {
...
//					int remain = remainingUtility; // FOR OPTIMIZATION
                    ...
                    UtilityListPHMN utilityListOfItem = mapItemToUtilityList.get(pair.item);

                    // Add a new Element to the utility list of this item corresponding to this transaction
                    ElementPHMN element = new ElementPHMN(tid, pair.utility, remainingUtility);
...
//		int compare = (int)( mapItemToItemInfo.get(item1).support - mapItemToItemInfo.get(item2).support);
     ...
     * @throws IOException
     */
    private void phm(int[] prefix, int prefixLength, UtilityListPHMN pUL, List<UtilityListPHMN> ULs, int minUtility)
            throws IOException {
        ...
        for (int i = 0; i < ULs.size(); i++) {
            ...
            if (X.sumIutils + X.sumRutils >= minUtility) {
                ...
                if (patternSize < maximumLength) {
                    ...
                    List<UtilityListPHMN> exULs = new ArrayList<UtilityListPHMN>();
                    // For each extension of p appearing
                    // after X according to the ascending order
                    for (int j = i + 1; j < ULs.size(); j++) {
                        UtilityListPHMN Y = ULs.get(j);
                        ...
                        UtilityListPHMN temp = construct(pUL, X, Y, minUtility);
    ...
    /********************* improve ***********************/
    private List<UtilityListPHMN> funcImprove(List<UtilityListPHMN> exULs, Integer item) {
        List<UtilityListPHMN> ress = new ArrayList<>();
        for (UtilityListPHMN utilityListPHM : exULs) {
            UtilityListPHMN utilityListPHM1 = new UtilityListPHMN(utilityListPHM.item);
            ...
            for (ElementPHMN element : utilityListPHM.elements) {
                Map<Integer, Integer> mmap = new HashMap<>();
                mmap = mapTidtoMapItemToUtility.get(element.tid);
                if (mmap.get(item) != null) {
                    int rUtil = element.rutils - mmap.get(item);
                    ElementPHMN element1 = new ElementPHMN(element.tid, element.iutils, rUtil);
                    utilityListPHM1.addElement(element1);
                } else {
                    ElementPHMN element1 = new ElementPHMN(element.tid, element.iutils, element.rutils);
                    utilityListPHM1.addElement(element1);
                }
            }
...
//	}
     ...
     */
    private UtilityListPHMN construct(UtilityListPHMN P, UtilityListPHMN px, UtilityListPHMN py, int minUtility) {
        // create an empy utility list for pXY
        UtilityListPHMN pxyUL = new UtilityListPHMN(py.item);
        boolean prune = false;
        // ********** PHM *************
...
//		}
        ...
        for (ElementPHMN ex : px.elements) {
            // do a binary search to find element ey in py with tid = ex.tid
            ElementPHMN ey = findElementWithTID(py, ex.tid);
            if (ey == null) {
                //== new optimization - LA-prune == /
                if (ENABLE_LA_PRUNE) {
...
//
                ...
                ElementPHMN eXY = new ElementPHMN(ex.tid, ex.iutils + ey.iutils, ey.rutils);
                // add the new element to the utility list of pXY
                pxyUL.addElement(eXY);

            } else {
                // find the element in the utility list of p wih the same tid
                ElementPHMN e = findElementWithTID(P, ex.tid);
                if (e != null) {
                    // ********** PHM *************
                    // check the periodicity
                    int periodicity = ex.tid - lastTid;
                    ...
                    // Create new element
                    ElementPHMN eXY = new ElementPHMN(ex.tid, ex.iutils + ey.iutils - e.iutils,
                            ey.rutils);
                    // add the new element to the utility list of pXY
                    pxyUL.addElement(eXY);
                }
...
//		}
     ...
     */
    private ElementPHMN findElementWithTID(UtilityListPHMN ulist, int tid) {
        List<ElementPHMN> list = ulist.elements;
     ...
     */
    private void writeOut(int[] prefix, int prefixLength, UtilityListPHMN utilityList, double averagePeriodicity) throws IOException {
