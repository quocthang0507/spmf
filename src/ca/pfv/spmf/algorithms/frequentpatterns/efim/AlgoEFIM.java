...
//    	//======
     ...
     */
    private void backtrackingEFIM(ListObject<Transaction> transactionsOfP, ListInt itemsToKeep, ListInt itemsToExplore,
        ...
        for (int j = 0; j < itemsToExplore.size(); j++) {
            ...
            // For each transaction
            for (int w = 0; w < transactionsOfP.size(); w++) {
                Transaction transaction = transactionsOfP.get(w);
...
//		        }
    ...
    public void useUtilityBinArrayToCalculateSubtreeUtilityFirstTime(Dataset dataset) {
        ...
        // For each transaction
        for (int b = 0; b < dataset.getTransactions().size(); b++) {
            Transaction transaction = dataset.getTransactions().get(b);
     ...
     */
    private void useUtilityBinArraysToCalculateUpperBounds(ListObject<Transaction> transactionsPe, int j,
                                                           ListInt itemsToKeep) {
        ...
        // for each transaction
        for (int z = 0; z < transactionsPe.size(); z++) {
            Transaction transaction = transactionsPe.get(z);
