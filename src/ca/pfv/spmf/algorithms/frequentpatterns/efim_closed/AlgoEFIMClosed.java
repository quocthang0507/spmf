...
public class AlgoEFIMClosed {
     ...
     */
    public static void insertionSort(ListInt items,
    ...
    public Itemsets runAlgorithm(int minUtil, String inputPath,
        ...
        if (activateTransactionMerging) {
            ...
            dataset.getTransactions().sort(
                    new ComparatorObject<Transaction>() {
                        @Override
                        /**
            ...
            dataset.transactions = dataset.transactions.immutableSubList(
        ...
        // condition
        ListInt itemsToExplore = new ArrayListInt();
        // if subtree utility pruning is activated
        if (activateSubtreeUtilityPruning) {
            // for each item
            for (int z = 0; z < itemsToKeep.size(); z++) {
                int item = itemsToKeep.get(z);
     ...
     */
    private int backtrackingEFIM(ListObject<Transaction> transactionsOfP,
                                 ListInt itemsToKeep, ListInt itemsToExplore,
        ...
        for (int j = 0; j < itemsToExplore.size(); j++) {
            ...
            // "e"
            ListObject<Transaction> transactionsPe = new ArrayListObject<Transaction>();

            // variable to calculate the utility of P U {e}
            int utilityPe = 0;
            int supportPe = 0;

            ListObject<Transaction> nowEmptyTransactionsPe = new ArrayListObject<Transaction>(); // EFIM-closed
            ...
            // For each transaction
            for (int g = 0; g < transactionsOfP.size(); g++) {
                Transaction transaction = transactionsOfP.get(g);
//			for (Transaction transaction : transactionsOfP) {
            ...
            if (hasNoBackwardExtension(temp, prefixLength, transactionsPe, nowEmptyTransactionsPe, e)) {
                ...
                if (shouldJumpToClosure) {
                    ...
                    ListInt newItemsToKeep = new ArrayListInt();
                    // We will create the new list of primary items
                    ListInt newItemsToExplore = new ArrayListInt();
    ...
    private boolean hasNoBackwardExtension(int[] prefix, int prefixLength,
                                           ListObject<Transaction> transactionsPe,
                                           ListObject<Transaction> nowEmptyTransactions, int e) {
     ...
     */
    private boolean isItemInAllTransactions(ListObject<Transaction> transactionsPe, int item) {

        for (int g = 0; g < transactionsPe.size(); g++) {
            Transaction mergedTransaction = transactionsPe.get(g);
//		for (Transaction mergedTransaction : transactionsPe) {
    ...
    public void useUtilityBinArrayToCalculateLocalUtilityFirstTime(Dataset dataset) {
        ...
        // For each transaction
        for (int g = 0; g < dataset.getTransactions().size(); g++) {
            Transaction transaction = dataset.getTransactions().get(g);
//		for (Transaction transaction : dataset.getTransactions()) {
            // for each item
            for (int z = 0; z < transaction.getItems().length; z++) {
                int item = transaction.getItems()[z];
//			for (int item : transaction.getItems()) {
    ...
    public void useUtilityBinArrayToCalculateSubtreeUtilityFirstTime(Dataset dataset) {
        ...
        for (int g = 0; g < transactions.size(); g++) {
            Transaction transaction = transactions.get(g);
//		for (Transaction transaction : dataset.getTransactions()) {
     ...
     * @return
     */
    private int useUtilityBinArraysToCalculateUpperBounds(ListObject<Transaction> transactionsPe,
                                                          int j, ListInt itemsToKeep) {
...
//		long initialTime = System.currentTimeMillis();
        ...
        // for each transaction
        for (int g = 0; g < transactionsPe.size(); g++) {
            Transaction transaction = transactionsPe.get(g);
