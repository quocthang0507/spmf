...
public class PFPTree {
    ...
    Map<Integer, PFPNode> mapItemNodes = new HashMap<Integer, PFPNode>();

    // Map that indicates the last node for each item using the node links
    // key: item   value: an fp tree node
    Map<Integer, PFPNode> mapItemLastNode = new HashMap<Integer, PFPNode>();

    // root of the tree
    PFPNode root = new PFPNode(); // null node
    ...
    public void addTransaction(List<Integer> transaction) {
        PFPNode currentNode = root;
        // For each item in the transaction
        for (Integer item : transaction) {
            // look if there is a node already in the FP-Tree
            PFPNode child = currentNode.getChildWithID(item);
            if (child == null) {
                // there is no node, we create a new one
                PFPNode newNode = new PFPNode();
                newNode.itemID = item;
     ...
     */
    private void fixNodeLinks(Integer item, PFPNode newNode) {
        // get the latest node in the tree with this item
        PFPNode lastNode = mapItemLastNode.get(item);
        if (lastNode != null) {
        ...
        PFPNode headernode = mapItemNodes.get(item);
     ...
     */
    void addPrefixPath(List<PFPNode> prefixPath, Map<Integer, Integer> mapSupportBeta, int relativeMinsupp) {
        ...
        for (int i = prefixPath.size() - 1; i >= 1; i--) {
            PFPNode pathItem = prefixPath.get(i);
            // if the item is not frequent we skip it
            if (mapSupportBeta.get(pathItem.itemID) >= relativeMinsupp) {

                // look if there is a node already in the FP-Tree
                PFPNode child = currentNode.getChildWithID(pathItem.itemID);
                if (child == null) {
                    // there is no node, we create a new one
                    PFPNode newNode = new PFPNode();
     ...
     * @param relativeMinsupp
     */
    void addPrefixPathGRGrowth(List<PFPNode> prefixPath, Map<Integer, Integer> mapSupportBeta, int relativeMinsupp) {
        // the first element of the prefix path contains the path support
        int pathCount = prefixPath.get(0).counter;
