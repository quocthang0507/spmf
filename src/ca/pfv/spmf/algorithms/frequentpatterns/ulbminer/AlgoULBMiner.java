...
public class AlgoULBMiner {
    ...
    public void runAlgorithm(String input, String output, int minUtility) throws IOException {
        ...
        mapItemToTWU = new LMapIntToLong(1000);

        // We create a map to store the support of each item
        mapItemToSupport = new LMapIntToInt(1000);
        ...
        try {
            ...
            while ((thisLine = myInput.readLine()) != null) {
                ...
                for (int i = 0; i < items.length; i++) {
                    ...
                    twu = (twu == -1) ? transactionUtility : twu + transactionUtility;
        ...
        ListInt promisingItems = new ArrayListInt();
        // For each item
        EntryIterator iterPlus = mapItemToTWU.iterator();
        while (iterPlus.hasNext()) {
            int item = iterPlus.next().getKey();
//		}
//		for(Integer item: mapItemToTWU.keySet()){
        ...
        promisingItems.sort(new ComparatorInt() {
        ...
        mapFMAP = new LMapIntToObject<MapIntToLong>(promisingItems.size());
        mapFMAP.setRehashingEnabled(false);
        ...
        MapIntToInt mapItemToUtilityList = new LMapIntToInt();
        int endPosition = 0;

        // For each item
        for (int z = 0; z < promisingItems.size(); z++) {
            int item = promisingItems.get(z);
...
//						return o1.item - o2.item;
                ...
                for (int i = 0; i < revisedTransaction.size(); i++) {
                    ...
                    int utilityListPosition = mapItemToUtilityList.get(pair.item);
                    ...
                    MapIntToLong mapFMAPItem = mapFMAP.get(pair.item);
                    if (mapFMAPItem == null) {
                        mapFMAPItem = new LMapIntToLong();
...
//						}
                        mapFMAPItem.getAndIncreaseValueBy(pairAfter.item, newTWU);
        ...
        for (int X = previousStartPosition; X < previousEndPosition; X++) {
            ...
            if (sumIutils + sumRutils >= minUtility) {
                ...
                for (int Y = X + 1; Y < previousEndPosition; Y++) {
                    ...
                    MapIntToLong mapTWUF = mapFMAP.get(itemX);
                    if (mapTWUF != null) {
